...
export const processJob = async (jobId: string): Promise<void> => {
      ...
      async () => {
        ...
        if (jobIndex !== -1) {
          try {
            ...
            ioInstance.emit('jobCompleted', { id: jobId, result: imageUrl });
          } catch (error) {
  ...
  async function getRandomFoodImage(): Promise<{
    ...
    const url = `https://api.unsplash.com/photos/random?query=food&clent_id=${process.env.UNSPLASH_ACCESS_KEY}`;
